...
Module.register("newsfeed", {
  ...
  getDom: function () {
    ...
    if (this.config.feedUrl) {
      ...
      wrapper.innerHTML = this.translate("MODULE_CONFIG_CHANGED", {
        ...
        if (this.isShowingDescription) {
          for (let f = 0; f < this.config.startTags.length; f++) {
            if (
              ...
              this.newsItems[this.activeItem].description = this.newsItems[
                this.activeItem
              ].description.slice(
                this.config.startTags[f].length,
        ...
        if (this.isShowingDescription) {
          for (let f = 0; f < this.config.endTags.length; f++) {
            if (
              ...
              this.newsItems[this.activeItem].description = this.newsItems[
                this.activeItem
              ].description.slice(0, -this.config.endTags[f].length);
            }
          }
        }
      }
        ...
        for (var i in feedItems) {
          ...
          if (
            !(
              ...
              Date.now() - new Date(item.pubdate) > this.config.ignoreOlderThan
            )
          ) {
            newsItems.push(item);
          }
      ...
      newsItems = newsItems.filter(function (value) {
        for (var i = 0; i < this.config.prohibitedWords.length; i++) {
          if (
            value["title"]
              .toLowerCase()
              .indexOf(this.config.prohibitedWords[i].toLowerCase()) > -1
          ) {
            return false;
          }
        }
  ...
  notificationReceived: function (notification, payload, sender) {
    ...
    } else if (notification == "act") {
      this.hide(0);
      this.updateDom();
    } else if (notification == "EXITsig") {
      this.show(0);
      this.updateDom();
    }
  },
