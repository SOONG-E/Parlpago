...
Module.register("weatherforecast", {
  ...
  getDom: function () {
    ...
    if (
      this.config.appid === "" ||
      this.config.appid === "YOUR_OPENWEATHER_API_KEY"
    ) {
      wrapper.innerHTML =
    ...
    for (var f in this.forecast) {
      ...
      maxTempCell.innerHTML =
        forecast.maxTemp.replace(".", this.config.decimalSymbol) + degreeLabel;
      maxTempCell.className = "align-right bright max-temp";
      row.appendChild(maxTempCell);

      var minTempCell = document.createElement("td");
      minTempCell.innerHTML =
        forecast.minTemp.replace(".", this.config.decimalSymbol) + degreeLabel;
      ...
      if (this.config.showRainAmount) {
        ...
        if (isNaN(forecast.rain)) {
          ...
          if (config.units !== "imperial") {
            rainCell.innerHTML =
              parseFloat(forecast.rain)
                .toFixed(1)
                .replace(".", this.config.decimalSymbol) + " mm";
          } else {
            rainCell.innerHTML =
              (parseFloat(forecast.rain) / 25.4)
                .toFixed(2)
                .replace(".", this.config.decimalSymbol) + " in";
          }
        }
      ...
      if (this.readyState === 4) {
        if (this.status === 200) {
          ...
          if (self.config.forecastEndpoint === "forecast/daily") {
            ...
            Log.warn(
              self.name +
                ": Your AppID does not support long term forecasts. Switching to fallback endpoint."
            );
          }
  ...
  getParams: function () {
    ...
    if (this.config.forecastEndpoint === "forecast") {
      numberOfDays =
        this.config.maxNumberOfDays < 1 || this.config.maxNumberOfDays > 5
          ? 5
          : this.config.maxNumberOfDays;
      // don't get forecasts for the next day, as it would not represent the whole day
      numberOfDays =
        numberOfDays * 8 - (Math.round(new Date().getHours() / 3) % 8);
    } else {
      numberOfDays =
        this.config.maxNumberOfDays < 1 || this.config.maxNumberOfDays > 17
          ? 7
          : this.config.maxNumberOfDays;
    }
      ...
      if (day !== lastDay) {
        ...
        forecastData.maxTemp =
          forecast.temp.max > parseFloat(forecastData.maxTemp)
            ? this.roundValue(forecast.temp.max)
            : forecastData.maxTemp;
        ...
        forecastData.minTemp =
          forecast.temp.min < parseFloat(forecastData.minTemp)
            ? this.roundValue(forecast.temp.min)
            : forecastData.minTemp;
  ...
  processRain: function (forecast, allForecasts) {
    ...
    var checkDateTime = forecast.dt_txt
      ? moment(forecast.dt_txt, "YYYY-MM-DD hh:mm:ss")
      : moment(forecast.dt, "X");
    var daysForecasts = allForecasts.filter(function (item) {
      var itemDateTime = item.dt_txt
        ? moment(item.dt_txt, "YYYY-MM-DD hh:mm:ss")
        : moment(item.dt, "X");
      return (
        itemDateTime.isSame(checkDateTime, "day") && item.rain instanceof Object
      );
    });
